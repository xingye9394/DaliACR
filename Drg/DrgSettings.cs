...
public class DrgSettings
{
    public static DrgSettings Instance;
    /// <summary>
    ...
    {
        path = Path.Combine(settingPath, nameof(DrgSettings), ".json");
        if (!File.Exists(path))
        {
            Instance = new DrgSettings();
            Instance.Save();
            return;
        }
        try
        {
            Instance = JsonHelper.FromJson<DrgSettings>(File.ReadAllText(path));
        }
        catch (Exception e)
        {
            Instance = new();
